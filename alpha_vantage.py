...
def fetch_alpha_vantage(ticker):
    try:
        ...
        return {
            ...
            "pe_ratio": round(float(ov_data.get("PERatio", 0)), 2) if ov_data.get("PERatio") != "None" else None,
            "pb_ratio": round(float(ov_data.get("PriceToBookRatio", 0)), 2) if ov_data.get("PriceToBookRatio") != "None" else None,
            "dividend_yield": round(float(ov_data.get("DividendYield", 0)) * 100, 2) if ov_data.get("DividendYield") != "None" else 0,
            "52w_high": float(ov_data.get("52WeekHigh", 0)) if ov_data.get("52WeekHigh") != "None" else None,
            "52w_low": float(ov_data.get("52WeekLow", 0)) if ov_data.get("52WeekLow") != "None" else None,
            "current_price": float(ov_data.get("50DayMovingAverage", 0)) if ov_data.get("50DayMovingAverage") != "None" else None,
            "sector": ov_data.get("Sector", "N/A"),
            "roe": round(float(ov_data.get("ReturnOnEquityTTM", 0)) * 100, 2) if ov_data.get("ReturnOnEquityTTM") != "None" else None,
